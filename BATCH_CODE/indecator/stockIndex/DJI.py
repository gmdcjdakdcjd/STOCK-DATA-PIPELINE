...
class DJIDailyBatchOut:
    def __init__(self):
        ...
        print(f"[INFO] DJI pages_to_fetch = {self.pages_to_fetch}")
        ...
        try:
            ...
            for page in range(1, self.pages_to_fetch + 1):
                url = (
                    ...
                    f"?symbol=DJI@DJI&fdtc=0&page={page}"
                )
                ...
                if isinstance(data, dict) and "worldDayList" in data:
                    ...
                    print("[ERROR] DJI JSON 구조 이상:", data)
                    break
    ...
    def write_indicator(self, df):
        for idx, r in df.iterrows():
            ...
            print(f"[{tmnow}] #{idx + 1:04d} DJI > WRITE TXT OK")

    # ===============================================================
    # 3) 실행
    # ===============================================================
    def execute(self):
        print("[INFO] DJI Batch-Out 시작")
        df = self.read_DJI()

        if df.empty:
            print("[WARN] DJI 데이터 없음")
            return

        self.write_indicator(df)
        print("[INFO] DJI Batch-Out 완료")


# 실행부
if __name__ == "__main__":
    DJIDailyBatchOut().execute()
