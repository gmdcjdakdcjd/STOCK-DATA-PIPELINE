...
class TWSDailyBatchOut:
    def __init__(self):
        ...
        print(f"[INFO] TWS pages_to_fetch = {self.pages_to_fetch}")
        ...
        try:
            ...
            for page in range(1, self.pages_to_fetch + 1):
                url = (
                    ...
                    f"?symbol=TWS@TI01&fdtc=0&page={page}"
                )
                ...
                if isinstance(data, dict) and "worldDayList" in data:
                    ...
                    print("[ERROR] TWS JSON 구조 이상:", data)
                    break
    ...
    def write_indicator(self, df):
        for idx, r in df.iterrows():
            ...
            print(f"[{tmnow}] #{idx + 1:04d} TWS > WRITE TXT OK")

    # ===============================================================
    # 3) 실행
    # ===============================================================
    def execute(self):
        print("[INFO] TWS Batch-Out 시작")
        df = self.read_TWS()

        if df.empty:
            print("[WARN] TWS 데이터 없음")
            return

        self.write_indicator(df)
        print("[INFO] TWS Batch-Out 완료")


# 실행부
if __name__ == "__main__":
    TWSDailyBatchOut().execute()
