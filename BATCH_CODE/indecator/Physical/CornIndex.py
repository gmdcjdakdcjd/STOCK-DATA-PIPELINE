...
class CORNDailyBatchOut:
    def __init__(self):
        ...
        print(f"[INFO] CORN pages_to_fetch = {self.pages_to_fetch}")

    # -------------------------------------------------
    def read_CORN_page(self, page=1):
        try:
            url = (
                "https://finance.naver.com/marketindex/worldDailyQuote.naver"
                f"?marketindexCd=CMDT_C&fdtc=2&page={page}"
            )
            ...
            for row in rows:
                ...
                data.append(["CORN", date, change_amount, round(rate, 4), close])
    ...
    def execute(self):
        ...
        print("[INFO] CORN Batch-Out 완료")


if __name__ == "__main__":
    CORNDailyBatchOut().execute()
