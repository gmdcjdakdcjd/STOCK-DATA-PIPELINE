...
class BEANDailyBatchOut:
    def __init__(self):
        ...
        print(f"[INFO] BEAN pages_to_fetch = {self.pages_to_fetch}")

    # -------------------------------------------------
    def read_BEAN_page(self, page=1):
        try:
            url = (
                "https://finance.naver.com/marketindex/worldDailyQuote.naver"
                f"?marketindexCd=CMDT_S&fdtc=2&page={page}"
            )
            ...
            for row in rows:
                ...
                data.append(["BEAN", date, change_amount, round(rate, 4), close])
    ...
    def execute(self):
        ...
        print("[INFO] BEAN Batch-Out 완료")


if __name__ == "__main__":
   BEANDailyBatchOut().execute()
