...
class COTTONDailyBatchOut:
    def __init__(self):
        ...
        print(f"[INFO] COTTON pages_to_fetch = {self.pages_to_fetch}")


    # ------------------------------------------------------------------
    # 1) 국제 금(COTTON) 일별 시세 한 페이지 수집
    # ------------------------------------------------------------------
    def read_COTTON_daily(self, page=1):
        try:
            url = (
                "https://finance.naver.com/marketindex/worldDailyQuote.naver"
                f"?marketindexCd=CMDT_CT&fdtc=2&page={page}"
            )
            ...
            print("[ERROR] COTTON page error:", e)
            return pd.DataFrame()
    ...
    def collect_latest(self):
        ...
        for page in range(1, self.pages_to_fetch + 1):
            print(f"[INFO] COTTON page {page}/{self.pages_to_fetch}", end="\r")

            df = self.read_COTTON_daily(page)
            if df.empty:
                break
    ...
    def write_indicator(self, df):
        for idx, r in df.iterrows():
            ...
            print(f"[{tmnow}] #{idx + 1:04d} COTTON > WRITE TXT OK")

    # ------------------------------------------------------------------
    # 4) 실행
    # ------------------------------------------------------------------
    def execute(self):
        print("[INFO] COTTON Batch-Out 시작")
        df = self.collect_latest()

        if df.empty:
            print("[WARN] COTTON 데이터 없음")
            return

        self.write_indicator(df)
        print("[INFO] COTTON Batch-Out 완료")


if __name__ == "__main__":
    COTTONDailyBatchOut().execute()
