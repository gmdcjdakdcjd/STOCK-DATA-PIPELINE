...
class FXDailyBatchOut:
    ...
    # 1) HKDKRW 한 페이지 수집
    # -------------------------------------------------
    def read_fx_HKDKRW(self, page):
        url = (
            "https://finance.naver.com/marketindex/exchangeDailyQuote.naver"
            f"?marketindexCd=FX_HKDKRW&page={page}"
        )

        headers = {
            "User-Agent": "Mozilla/5.0",
            "Referer": "https://finance.naver.com/marketindex/exchangeDetail.naver?marketindexCd=FX_HKDKRW"
        }
    ...
    def collect_latest(self):
        ...
        for page in range(1, self.pages_to_fetch + 1):
            print(f"[INFO] HKDKRW page {page}/{self.pages_to_fetch}")
            df = self.read_fx_HKDKRW(page)
            if df.empty:
                break
            frames.append(df)
    ...
    def save_to_txt(self, df):
        ...
        with open(txt_path, "a", encoding="utf-8") as f:
            ...
            for idx, r in df.iterrows():
                ...
                print(f"[{tmnow}] #{idx + 1:04d} HKDKRW > WRITE INDICATOR OK")
    ...
    # -------------------------------------------------
    def execute(self):
        print("[INFO] HKDKRW Batch-Out 시작")
