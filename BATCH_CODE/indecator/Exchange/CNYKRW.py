...
class FXDailyBatchOut:
    ...
    # 1) CNYKRW 한 페이지 수집
    # -------------------------------------------------
    def read_fx_CNYKRW(self, page):
        url = (
            "https://finance.naver.com/marketindex/exchangeDailyQuote.naver"
            f"?marketindexCd=FX_CNYKRW&page={page}"
        )

        headers = {
            "User-Agent": "Mozilla/5.0",
            "Referer": "https://finance.naver.com/marketindex/exchangeDetail.naver?marketindexCd=FX_CNYKRW"
        }
    ...
    def collect_latest(self):
        ...
        for page in range(1, self.pages_to_fetch + 1):
            print(f"[INFO] CNYKRW page {page}/{self.pages_to_fetch}")
            df = self.read_fx_CNYKRW(page)
            if df.empty:
                break
            frames.append(df)
    ...
    def save_to_txt(self, df):
        ...
        with open(txt_path, "a", encoding="utf-8") as f:
            ...
            for idx, r in df.iterrows():
                ...
                print(f"[{tmnow}] #{idx + 1:04d} CNYKRW > WRITE INDICATOR OK")
    ...
    # -------------------------------------------------
    def execute(self):
        print("[INFO] CNYKRW Batch-Out 시작")
